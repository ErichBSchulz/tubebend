...
function calculateGeometry(params) {
  ...
  const bladeRadians = Math.asin(params.bladeLength / (params.bladeRadius * 2)) * 2;
  ...
  state.blade = {
      ...state.bladeCentre,
      startAngle: params.bladeAngle - bladeRadians * (1 - (params.bladeInsertion / 100)),
...
// then bending by the same ammount at the teeth for arc1.
  ...
  if (intersection !== null && intersection.x > state.bladeTip.x && intersection.y > params.upperIncisorY) {
      ...
      state.arcParams3.endAngle = finalAngle > state.arcParams3.startAngle ? finalAngle : state.arcParams3.startAngle;
  ...
  } else { // no contact between blade and tube
  ...
  const remainingRadians = params.tubeLength / params.tubeRadius - state.drawnTubeRadians;
  ...
  state.fiducial = { // rename all these to be prefixed by `fiducial`
...
function draw(state, appearance) {
  ...
  // teeth
  drawTooth({x: state.upperIncisorX, y: state.upperIncisorY, height: 10, lineWidth: 1});
  drawTooth({x: state.lowerIncisorX, y: state.lowerIncisorY, height: -10, lineWidth: 1});
  // blade
  drawArc(state.blade);
  ...
  if (appearance.showLabels) {
      label({x: state.lowerIncisorX, y: state.lowerIncisorY, text: 'Lower Incisor', alignment: 'left'});
      label({x: state.upperIncisorX, y: state.upperIncisorY, text: 'Upper Incisor', alignment: 'right'});
      label({x: state.bladeTip.x, y: state.bladeTip.y, text: 'Blade', alignment: 'above'});
      label({x: state.glottis.start.x, y: state.glottis.start.y,
        text: 'Glottis', alignment: 'left', offset: 5});
      label({x: state.tubeTip.x, y: state.tubeTip.y, text: 'Tube', alignment: 'below'});
  }
}
...
const lowerIncisor = rescale({ x: ctx.geometry.lowerIncisorX, y: ctx.geometry.lowerIncisorY});
const upperIncisor = rescale({ x: ctx.geometry.upperIncisorX, y: ctx.geometry.upperIncisorY});
